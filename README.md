...
## 🔄 Переключение режимов разработки/продакшена
...
### Как переключаться:
1. **Измени** нужные строки в `.env` файле
2. **Очисти кэш конфигурации:**
```bash
...
### Что происходит:
- **`APP_ENV=local`** → загружаются файлы с `localhost:5173` (Vite dev сервер с HMR)
- **`APP_ENV=production`** → загружаются файлы из `public/build/` (скомпилированные)

**Важно:** После изменения `.env` обязательно очищай кэш Laravel!
