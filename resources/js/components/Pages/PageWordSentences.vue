<template>
    <div id="page_list_sentences" class="page-word-sentences">
        <div class="container-fluid">
            ...
            <div class="flex items-center justify-between mb-6">
                <div class="space-y-1">
                    ...
                    <p class="text-muted-foreground">Управление предложениями и их переводами</p>
                </div>

                <div class="flex items-end space-x-3">
                    <!-- кнопки озвучки предложений -->
                    <div v-show="getCodeLearnLanguage2 == 'en'" class="box-sound">
                        <div
                            v-if="!speak.start"
                            id="block_repeat"
                        ...
                        >
                            <div class="text-center">
                                ...
                                <div
                                    class="block_input_repeat flex items-center justify-center space-x-2"
                                >
                                    <input
                                        :checked="speak.repeat_bool"
                                    ...
                                    <input
                                        ...
                                        class="form-control number_repeat"
                                        min="1"
                                        max="10"
                                        type="number"
                                    />
                    ...
                    <!-- Создать предложение -->
                    <button class="btn btn-primary" @click="openModalCreateSentence">
                        <svg
                            class="w-4 h-4 mr-2"
                            fill="none"
                            stroke="currentColor"
                    ...
                    <!-- learn sentences -->
                    <button class="btn btn-secondary" @click="openLearnModal()">
                        <svg
                            class="w-4 h-4 mr-2"
                            fill="none"
                            stroke="currentColor"
            ...
            <div v-if="table.rows.length" class="table_wrapper">
                <vue-good-table
                    ...
                    style-class="vgt-table sentence"
                    @on-search="onSearch"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        ...
                        <button
                            ...
                            class="btn-close"
                            aria-label="Close"
                            @click="closeCreateSentenceModal"
                        ></button>
                    </div>
                    <!-- body -->
                    <div class="modal-body">
                        <form action="#" class="space-y-4">
                            ...
                            <div class="form-group">
                                ...
                                ></textarea>
                                <div v-if="sentenceError" class="text-danger text-sm mt-1">
                                    {{ sentenceError }}
                                </div>
                                <div v-if="!$v.new_sentence.required" class="invalid-feedback">
                                    {{ $t('all.field_is_empty') }}
                                </div>
                                <div v-if="!$v.new_sentence.minLength" class="invalid-feedback">
                            ...
                            <div class="form-group">
                                ...
                                ></textarea>
                                <div
                                    v-if="!$v.translation_sentence.required"
                                    class="invalid-feedback"
                                >
                                    {{ $t('all.field_is_empty') }}
                                </div>
                                <div
                                    v-if="!$v.translation_sentence.minLength"
                                    class="invalid-feedback"
                                >
                                    {{ $t('all.number_of_characters') }}
                                    {{ translation_sentence.length }}
                                    {{ $t('all.less_needed') }}
                                </div>
                            </div>
                        </form>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        ...
                        <button
                            ...
                            class="btn-close"
                            aria-label="Close"
                            @click="closeUpdateSentenceModal"
                        ></button>
                    </div>
                    <!-- body -->
                    <div class="modal-body">
        ...
        <ModalLearnSentence ref="modalLearnSentence"></ModalLearnSentence>
    </div>
</template>
    ...
    export default {
                        ...
                        },
                        {
                            ...
                            field: val => {
                                return (
                                    '<a data-id=' +
                                    val.but +
                                    ' class="btn btn-warning btn_sentence" role="button"> <span class="fa fa-edit"></span> </a>'
                                );
                            },
                        },
                    ],
        ...
        methods: {
            async bindCheckboxSound(sentence_id, status) {
                try {
                    ...
                    await this.$http.post(
                        `${this.$http.webUrl()}sentence/bind-checkbox-sound`,
                        data
                    );
                } catch (e) {
                    console.error('Error in bindCheckboxSound:', e);
                }
            },
            // --- предложения
            async loadSentences() {
                try {
                    ...
                    console.error('Error in loadSentences:', e);
                }
            ...
            handleServerError(response) {
                // валидации
                if (response.error.sentence) {
                    this.sentenceError = response.error.sentence[0];
                }
            ...
            async createSentence() {
                ...
                try {
                    ...
                    if (this.checkSuccess(response)) {
                        this.initialData();
                        // Закрываем модалку создания предложения
                        this.closeModalById('create_sentence');
                    }
                } catch (e) {
                    console.error('Error in createSentence:', e);
                }
            },
            ...
            async updateSentence() {
                ...
                try {
                    // showAlert = false - не показываем стандартные alert'ы
                    const response = await this.$http.post(
                        `${this.$http.webUrl()}sentence/update-sentence`,
                        data,
                        {},
                        false
                    );
                    ...
                    if (this.checkSuccess(response)) {
                        this.initialData();
                        // Закрываем модалку обновления предложения только при успехе
                        this.closeModalById('update_sentence');
                    ...
                    $('.btn_sentence').on('click', e => {
                        const queryObj =
                            $(e.target).prop('tagName') !== 'A'
                                ? $(e.target).parent()
                                : $(e.target);
                        const id = queryObj.attr('data-id');
                        const row = this.getSentenceCollection(id);
