    ...
    methods: {
        ...
        async searchWord(word) {
            try {
                ...
                console.error('Error in searchWord:', e);
            }
        ...
        searchHelpWord(word) {
            const string = word;
            const last_word = string.split(' ').pop();

            if (last_word.trim() !== '') {
                // найти в базе подходящие слова
                this.searchWord(last_word);
            }
            const dynamic_line = this.help_dynamic.split(' ');
        ...
        keysOnInputField() {
            const elem = document.getElementsByClassName('entry-field-help');
